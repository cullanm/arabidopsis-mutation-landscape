        ...
        if (ready_to_flush and read.reference_start > cur_pos) or num_read == total_reads:
            ...
            for var in to_flush:
                ...
                if args.all or (v['f_read1_sup'] > 0 and v['r_read1_sup'] > 0): # only output if support in both strands
                    output_vals = [contig] + list(var) + [v['f_read1_sup'], f_read1_cov, v['r_read1_sup'], r_read1_cov, v['f_sup'], f_cov, v['r_sup'], r_cov,                                                           v['f_read1_conc'], v['r_read1_conc'], v['mq'], v['bq'], max_mr]
