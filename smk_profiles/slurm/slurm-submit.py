...
################ Cullan changes
# 7) create output and error filenames and paths. Cullan modified
job_name = job_properties["rule"]
wildcards = job_properties['wildcards']
for w in ['group', 'subgroup', 'rep']:
    if w in wildcards:
        job_name += '_' + wildcards[w]
sbatch_options["output"] = f'logs/slurm/{job_name}.out'
sbatch_options["error"] = f'logs/slurm/{job_name}.err'
sbatch_options["job-name"] = os.getcwd().split('/')[-1] + '_' + job_name
...
# if mem is 0 (default), set it to 1.5G * the number of cpus
if 'mem' in sbatch_options and int(sbatch_options['mem']) == 0:
    sbatch_options['mem'] = str(1536 * int(sbatch_options['cpus-per-task']))

#with open('slurmtest.txt', 'a') as f:
#    f.write(str(job_properties) + '\n')
#    f.write(str(sbatch_options) + '\n')
